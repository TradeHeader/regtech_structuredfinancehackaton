...
public class ObligationsTypeFormatValidator implements Validator<Obligations> {
	...
	public ValidationResult<Obligations> validate(RosettaPath path, Obligations o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Obligations", ValidationType.TYPE_FORMAT, "Obligations", path, "", error);
		}
		return success("Obligations", ValidationType.TYPE_FORMAT, "Obligations", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Obligations", ValidationType.TYPE_FORMAT, "Obligations", path, "", res.getError());
				}
				return success("Obligations", ValidationType.TYPE_FORMAT, "Obligations", path, "");
			})
			.collect(toList());
