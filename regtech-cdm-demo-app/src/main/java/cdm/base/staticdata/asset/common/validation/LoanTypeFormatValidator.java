...
public class LoanTypeFormatValidator implements Validator<Loan> {
	...
	public ValidationResult<Loan> validate(RosettaPath path, Loan o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Loan", ValidationType.TYPE_FORMAT, "Loan", path, "", error);
		}
		return success("Loan", ValidationType.TYPE_FORMAT, "Loan", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Loan", ValidationType.TYPE_FORMAT, "Loan", path, "", res.getError());
				}
				return success("Loan", ValidationType.TYPE_FORMAT, "Loan", path, "");
			})
			.collect(toList());
