...
public class PayerReceiverTypeFormatValidator implements Validator<PayerReceiver> {
	...
	public ValidationResult<PayerReceiver> validate(RosettaPath path, PayerReceiver o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PayerReceiver", ValidationType.TYPE_FORMAT, "PayerReceiver", path, "", error);
		}
		return success("PayerReceiver", ValidationType.TYPE_FORMAT, "PayerReceiver", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PayerReceiver", ValidationType.TYPE_FORMAT, "PayerReceiver", path, "", res.getError());
				}
				return success("PayerReceiver", ValidationType.TYPE_FORMAT, "PayerReceiver", path, "");
			})
			.collect(toList());
