...
public class AccountTypeFormatValidator implements Validator<Account> {
	...
	public ValidationResult<Account> validate(RosettaPath path, Account o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Account", ValidationType.TYPE_FORMAT, "Account", path, "", error);
		}
		return success("Account", ValidationType.TYPE_FORMAT, "Account", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Account", ValidationType.TYPE_FORMAT, "Account", path, "", res.getError());
				}
				return success("Account", ValidationType.TYPE_FORMAT, "Account", path, "");
			})
			.collect(toList());
