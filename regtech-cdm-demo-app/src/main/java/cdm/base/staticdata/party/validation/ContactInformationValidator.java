...
public class ContactInformationValidator implements Validator<ContactInformation> {
	...
	public ValidationResult<ContactInformation> validate(RosettaPath path, ContactInformation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ContactInformation", ValidationType.CARDINALITY, "ContactInformation", path, "", error);
		}
		return success("ContactInformation", ValidationType.CARDINALITY, "ContactInformation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ContactInformation", ValidationType.CARDINALITY, "ContactInformation", path, "", res.getError());
				}
				return success("ContactInformation", ValidationType.CARDINALITY, "ContactInformation", path, "");
			})
			.collect(toList());
