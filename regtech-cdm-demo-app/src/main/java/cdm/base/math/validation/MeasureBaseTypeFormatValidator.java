...
public class MeasureBaseTypeFormatValidator implements Validator<MeasureBase> {
	...
	public ValidationResult<MeasureBase> validate(RosettaPath path, MeasureBase o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MeasureBase", ValidationType.TYPE_FORMAT, "MeasureBase", path, "", error);
		}
		return success("MeasureBase", ValidationType.TYPE_FORMAT, "MeasureBase", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MeasureBase", ValidationType.TYPE_FORMAT, "MeasureBase", path, "", res.getError());
				}
				return success("MeasureBase", ValidationType.TYPE_FORMAT, "MeasureBase", path, "");
			})
			.collect(toList());
