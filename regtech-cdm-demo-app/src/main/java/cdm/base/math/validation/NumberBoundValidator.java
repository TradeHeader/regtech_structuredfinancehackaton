...
public class NumberBoundValidator implements Validator<NumberBound> {
	...
	public ValidationResult<NumberBound> validate(RosettaPath path, NumberBound o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("NumberBound", ValidationType.CARDINALITY, "NumberBound", path, "", error);
		}
		return success("NumberBound", ValidationType.CARDINALITY, "NumberBound", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("NumberBound", ValidationType.CARDINALITY, "NumberBound", path, "", res.getError());
				}
				return success("NumberBound", ValidationType.CARDINALITY, "NumberBound", path, "");
			})
			.collect(toList());
