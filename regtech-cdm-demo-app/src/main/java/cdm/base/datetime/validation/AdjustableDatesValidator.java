...
public class AdjustableDatesValidator implements Validator<AdjustableDates> {
	...
	public ValidationResult<AdjustableDates> validate(RosettaPath path, AdjustableDates o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AdjustableDates", ValidationType.CARDINALITY, "AdjustableDates", path, "", error);
		}
		return success("AdjustableDates", ValidationType.CARDINALITY, "AdjustableDates", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustableDates", ValidationType.CARDINALITY, "AdjustableDates", path, "", res.getError());
				}
				return success("AdjustableDates", ValidationType.CARDINALITY, "AdjustableDates", path, "");
			})
			.collect(toList());
