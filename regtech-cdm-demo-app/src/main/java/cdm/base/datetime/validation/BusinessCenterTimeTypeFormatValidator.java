...
public class BusinessCenterTimeTypeFormatValidator implements Validator<BusinessCenterTime> {
	...
	public ValidationResult<BusinessCenterTime> validate(RosettaPath path, BusinessCenterTime o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("BusinessCenterTime", ValidationType.TYPE_FORMAT, "BusinessCenterTime", path, "", error);
		}
		return success("BusinessCenterTime", ValidationType.TYPE_FORMAT, "BusinessCenterTime", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("BusinessCenterTime", ValidationType.TYPE_FORMAT, "BusinessCenterTime", path, "", res.getError());
				}
				return success("BusinessCenterTime", ValidationType.TYPE_FORMAT, "BusinessCenterTime", path, "");
			})
			.collect(toList());
