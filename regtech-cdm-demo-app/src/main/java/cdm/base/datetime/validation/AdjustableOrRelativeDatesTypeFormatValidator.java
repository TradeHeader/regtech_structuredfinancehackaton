...
public class AdjustableOrRelativeDatesTypeFormatValidator implements Validator<AdjustableOrRelativeDates> {
	...
	public ValidationResult<AdjustableOrRelativeDates> validate(RosettaPath path, AdjustableOrRelativeDates o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AdjustableOrRelativeDates", ValidationType.TYPE_FORMAT, "AdjustableOrRelativeDates", path, "", error);
		}
		return success("AdjustableOrRelativeDates", ValidationType.TYPE_FORMAT, "AdjustableOrRelativeDates", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AdjustableOrRelativeDates", ValidationType.TYPE_FORMAT, "AdjustableOrRelativeDates", path, "", res.getError());
				}
				return success("AdjustableOrRelativeDates", ValidationType.TYPE_FORMAT, "AdjustableOrRelativeDates", path, "");
			})
			.collect(toList());
