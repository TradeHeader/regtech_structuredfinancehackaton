...
public interface OffsetDayType extends Validator<Offset> {
	...
	class Default implements OffsetDayType {
		...
		private ComparisonResult executeDataRule(Offset offset) {
			try {
				if (notEqual(MapperS.of(offset).<PeriodEnum>map("getPeriod", period -> period.getPeriod()), MapperS.of(PeriodEnum.D), CardinalityOperator.Any).or(areEqual(MapperS.of(offset).<Integer>map("getPeriodMultiplier", period -> period.getPeriodMultiplier()), MapperS.of(0), CardinalityOperator.All)).getOrDefault(false)) {
