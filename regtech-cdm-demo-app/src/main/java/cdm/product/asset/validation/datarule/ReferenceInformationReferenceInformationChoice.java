...
public interface ReferenceInformationReferenceInformationChoice extends Validator<ReferenceInformation> {
	...
	ValidationResult<ReferenceInformation> validate(RosettaPath path, ReferenceInformation referenceInformation);
	
	class Default implements ReferenceInformationReferenceInformationChoice {
	
		@Override
		public ValidationResult<ReferenceInformation> validate(RosettaPath path, ReferenceInformation referenceInformation) {
			ComparisonResult result = executeDataRule(referenceInformation);
			if (result.get()) {
				return ValidationResult.success(NAME, ValidationResult.ValidationType.DATA_RULE, "ReferenceInformation", path, DEFINITION);
			}
			...
			return ValidationResult.failure(NAME, ValidationType.DATA_RULE, "ReferenceInformation", path, DEFINITION, failureMessage);
		}
