...
public class FloatingAmountProvisionsTypeFormatValidator implements Validator<FloatingAmountProvisions> {
	...
	public ValidationResult<FloatingAmountProvisions> validate(RosettaPath path, FloatingAmountProvisions o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FloatingAmountProvisions", ValidationType.TYPE_FORMAT, "FloatingAmountProvisions", path, "", error);
		}
		return success("FloatingAmountProvisions", ValidationType.TYPE_FORMAT, "FloatingAmountProvisions", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingAmountProvisions", ValidationType.TYPE_FORMAT, "FloatingAmountProvisions", path, "", res.getError());
				}
				return success("FloatingAmountProvisions", ValidationType.TYPE_FORMAT, "FloatingAmountProvisions", path, "");
			})
			.collect(toList());
