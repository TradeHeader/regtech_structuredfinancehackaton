...
public class FloatingRateDefinitionTypeFormatValidator implements Validator<FloatingRateDefinition> {
	...
	public ValidationResult<FloatingRateDefinition> validate(RosettaPath path, FloatingRateDefinition o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FloatingRateDefinition", ValidationType.TYPE_FORMAT, "FloatingRateDefinition", path, "", error);
		}
		return success("FloatingRateDefinition", ValidationType.TYPE_FORMAT, "FloatingRateDefinition", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FloatingRateDefinition", ValidationType.TYPE_FORMAT, "FloatingRateDefinition", path, "", res.getError());
				}
				return success("FloatingRateDefinition", ValidationType.TYPE_FORMAT, "FloatingRateDefinition", path, "");
			})
			.collect(toList());
