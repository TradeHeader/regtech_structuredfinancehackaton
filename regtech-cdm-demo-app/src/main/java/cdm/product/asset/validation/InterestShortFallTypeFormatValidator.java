...
public class InterestShortFallTypeFormatValidator implements Validator<InterestShortFall> {
	...
	public ValidationResult<InterestShortFall> validate(RosettaPath path, InterestShortFall o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InterestShortFall", ValidationType.TYPE_FORMAT, "InterestShortFall", path, "", error);
		}
		return success("InterestShortFall", ValidationType.TYPE_FORMAT, "InterestShortFall", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InterestShortFall", ValidationType.TYPE_FORMAT, "InterestShortFall", path, "", res.getError());
				}
				return success("InterestShortFall", ValidationType.TYPE_FORMAT, "InterestShortFall", path, "");
			})
			.collect(toList());
