...
public class DiscountingMethodTypeFormatValidator implements Validator<DiscountingMethod> {
	...
	public ValidationResult<DiscountingMethod> validate(RosettaPath path, DiscountingMethod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("DiscountingMethod", ValidationType.TYPE_FORMAT, "DiscountingMethod", path, "", error);
		}
		return success("DiscountingMethod", ValidationType.TYPE_FORMAT, "DiscountingMethod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("DiscountingMethod", ValidationType.TYPE_FORMAT, "DiscountingMethod", path, "", res.getError());
				}
				return success("DiscountingMethod", ValidationType.TYPE_FORMAT, "DiscountingMethod", path, "");
			})
			.collect(toList());
