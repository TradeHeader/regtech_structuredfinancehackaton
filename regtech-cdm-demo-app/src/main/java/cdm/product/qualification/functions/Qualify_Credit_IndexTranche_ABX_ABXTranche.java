...
@ImplementedBy(Qualify_Credit_IndexTranche_ABX_ABXTranche.Qualify_Credit_IndexTranche_ABX_ABXTrancheDefault.class)
public abstract class Qualify_Credit_IndexTranche_ABX_ABXTranche implements RosettaFunction {
	...
	*/
	...
	public static class Qualify_Credit_IndexTranche_ABX_ABXTrancheDefault extends Qualify_Credit_IndexTranche_ABX_ABXTranche {
		@Override
		...
		protected Boolean assignOutput(Boolean is_product, TradeState tradeState) {
			is_product = ComparisonResult.of(MapperS.of(qualify_CreditDefaultSwap_IndexTranche.evaluate(economicTerms(tradeState).get()))).and(areEqual(contractualTermsSupplementType(tradeState), MapperS.of(ContractualSupplementTypeEnum.ABX_TRANCHE), CardinalityOperator.Any)).and(notExists(obligations(tradeState).<Boolean>map("getNotContingent", _obligations -> _obligations.getNotContingent())).or(areEqual(obligations(tradeState).<Boolean>map("getNotContingent", _obligations -> _obligations.getNotContingent()), MapperS.of(true), CardinalityOperator.All))).get();
