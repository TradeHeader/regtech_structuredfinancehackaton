...
@ImplementedBy(Qualify_Credit_IndexTranche_LCDX_LCDXTranche.Qualify_Credit_IndexTranche_LCDX_LCDXTrancheDefault.class)
public abstract class Qualify_Credit_IndexTranche_LCDX_LCDXTranche implements RosettaFunction {
	...
	*/
	...
	public static class Qualify_Credit_IndexTranche_LCDX_LCDXTrancheDefault extends Qualify_Credit_IndexTranche_LCDX_LCDXTranche {
		@Override
		...
		protected Boolean assignOutput(Boolean is_product, TradeState tradeState) {
			is_product = ComparisonResult.of(MapperS.of(qualify_CreditDefaultSwap_IndexTranche.evaluate(economicTerms(tradeState).get()))).and(areEqual(contractualTermsSupplementType(tradeState), MapperS.of(ContractualSupplementTypeEnum.LCDX_TRANCHE), CardinalityOperator.Any)).and(notExists(obligations(tradeState).<Boolean>map("getNotContingent", _obligations -> _obligations.getNotContingent())).or(areEqual(obligations(tradeState).<Boolean>map("getNotContingent", _obligations -> _obligations.getNotContingent()), MapperS.of(true), CardinalityOperator.All))).get();
