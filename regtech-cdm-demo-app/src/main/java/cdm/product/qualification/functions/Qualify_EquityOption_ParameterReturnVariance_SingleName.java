	...
	*/
	...
	public static class Qualify_EquityOption_ParameterReturnVariance_SingleNameDefault extends Qualify_EquityOption_ParameterReturnVariance_SingleName {
		...
		protected Boolean assignOutput(Boolean is_product, EconomicTerms economicTerms) {
			is_product = onlyExists(Arrays.asList(MapperS.of(economicTerms).<Payout>map("getPayout", _economicTerms -> _economicTerms.getPayout()).<OptionPayout>mapC("getOptionPayout", payout -> payout.getOptionPayout()))).and(areEqual(MapperS.of(qualify_EquitySwap_ParameterReturnVariance_SingleName.evaluate(underlierEconomicTerms(economicTerms).get())), MapperS.of(true), CardinalityOperator.All)).get();
