...
@ImplementedBy(Qualify_Credit_SingleName_Loans_LCDS.Qualify_Credit_SingleName_Loans_LCDSDefault.class)
public abstract class Qualify_Credit_SingleName_Loans_LCDS implements RosettaFunction {
	...
	*/
	...
	public static class Qualify_Credit_SingleName_Loans_LCDSDefault extends Qualify_Credit_SingleName_Loans_LCDS {
		@Override
		...
		protected Boolean assignOutput(Boolean is_product, TradeState tradeState) {
			is_product = ComparisonResult.of(MapperS.of(qualify_CreditDefaultSwap_Loan.evaluate(economicTerms(tradeState).get()))).and(areEqual(contractualTermsSupplementType(tradeState), MapperS.of(ContractualSupplementTypeEnum.STANDARD_LCDS), CardinalityOperator.Any)).get();
