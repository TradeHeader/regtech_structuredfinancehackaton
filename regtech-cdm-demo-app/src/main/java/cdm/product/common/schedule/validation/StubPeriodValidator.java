...
public class StubPeriodValidator implements Validator<StubPeriod> {
	...
	public ValidationResult<StubPeriod> validate(RosettaPath path, StubPeriod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("StubPeriod", ValidationType.CARDINALITY, "StubPeriod", path, "", error);
		}
		return success("StubPeriod", ValidationType.CARDINALITY, "StubPeriod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("StubPeriod", ValidationType.CARDINALITY, "StubPeriod", path, "", res.getError());
				}
				return success("StubPeriod", ValidationType.CARDINALITY, "StubPeriod", path, "");
			})
			.collect(toList());
