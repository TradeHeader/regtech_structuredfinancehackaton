...
public class LagValidator implements Validator<Lag> {
	...
	public ValidationResult<Lag> validate(RosettaPath path, Lag o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Lag", ValidationType.CARDINALITY, "Lag", path, "", error);
		}
		return success("Lag", ValidationType.CARDINALITY, "Lag", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Lag", ValidationType.CARDINALITY, "Lag", path, "", res.getError());
				}
				return success("Lag", ValidationType.CARDINALITY, "Lag", path, "");
			})
			.collect(toList());
