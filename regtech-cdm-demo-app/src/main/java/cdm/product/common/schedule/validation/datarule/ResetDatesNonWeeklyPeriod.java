...
public interface ResetDatesNonWeeklyPeriod extends Validator<ResetDates> {
	...
	class Default implements ResetDatesNonWeeklyPeriod {
		...
		private ComparisonResult executeDataRule(ResetDates resetDates) {
			try {
				if (notEqual(MapperS.of(resetDates).<ResetFrequency>map("getResetFrequency", _resetDates -> _resetDates.getResetFrequency()).<PeriodExtendedEnum>map("getPeriod", frequency -> frequency.getPeriod()), MapperS.of(PeriodExtendedEnum.W), CardinalityOperator.Any).getOrDefault(false)) {
