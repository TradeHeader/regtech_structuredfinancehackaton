...
public class AveragingPeriodTypeFormatValidator implements Validator<AveragingPeriod> {
	...
	public ValidationResult<AveragingPeriod> validate(RosettaPath path, AveragingPeriod o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AveragingPeriod", ValidationType.TYPE_FORMAT, "AveragingPeriod", path, "", error);
		}
		return success("AveragingPeriod", ValidationType.TYPE_FORMAT, "AveragingPeriod", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AveragingPeriod", ValidationType.TYPE_FORMAT, "AveragingPeriod", path, "", res.getError());
				}
				return success("AveragingPeriod", ValidationType.TYPE_FORMAT, "AveragingPeriod", path, "");
			})
			.collect(toList());
