...
public class ResetFrequencyTypeFormatValidator implements Validator<ResetFrequency> {
	...
	public ValidationResult<ResetFrequency> validate(RosettaPath path, ResetFrequency o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ResetFrequency", ValidationType.TYPE_FORMAT, "ResetFrequency", path, "", error);
		}
		return success("ResetFrequency", ValidationType.TYPE_FORMAT, "ResetFrequency", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResetFrequency", ValidationType.TYPE_FORMAT, "ResetFrequency", path, "", res.getError());
				}
				return success("ResetFrequency", ValidationType.TYPE_FORMAT, "ResetFrequency", path, "");
			})
			.collect(toList());
