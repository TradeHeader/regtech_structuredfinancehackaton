...
public class PricingDatesValidator implements Validator<PricingDates> {
	...
	public ValidationResult<PricingDates> validate(RosettaPath path, PricingDates o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PricingDates", ValidationType.CARDINALITY, "PricingDates", path, "", error);
		}
		return success("PricingDates", ValidationType.CARDINALITY, "PricingDates", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PricingDates", ValidationType.CARDINALITY, "PricingDates", path, "", res.getError());
				}
				return success("PricingDates", ValidationType.CARDINALITY, "PricingDates", path, "");
			})
			.collect(toList());
