...
public class QuantityMultiplierValidator implements Validator<QuantityMultiplier> {
	...
	public ValidationResult<QuantityMultiplier> validate(RosettaPath path, QuantityMultiplier o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("QuantityMultiplier", ValidationType.CARDINALITY, "QuantityMultiplier", path, "", error);
		}
		return success("QuantityMultiplier", ValidationType.CARDINALITY, "QuantityMultiplier", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("QuantityMultiplier", ValidationType.CARDINALITY, "QuantityMultiplier", path, "", res.getError());
				}
				return success("QuantityMultiplier", ValidationType.CARDINALITY, "QuantityMultiplier", path, "");
			})
			.collect(toList());
