...
public class ExtendibleProvisionTypeFormatValidator implements Validator<ExtendibleProvision> {
	...
	public ValidationResult<ExtendibleProvision> validate(RosettaPath path, ExtendibleProvision o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExtendibleProvision", ValidationType.TYPE_FORMAT, "ExtendibleProvision", path, "", error);
		}
		return success("ExtendibleProvision", ValidationType.TYPE_FORMAT, "ExtendibleProvision", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExtendibleProvision", ValidationType.TYPE_FORMAT, "ExtendibleProvision", path, "", res.getError());
				}
				return success("ExtendibleProvision", ValidationType.TYPE_FORMAT, "ExtendibleProvision", path, "");
			})
			.collect(toList());
