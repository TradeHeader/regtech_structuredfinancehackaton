...
public interface CollateralCollateralExists extends Validator<Collateral> {
	...
	class Default implements CollateralCollateralExists {
		...
		private ComparisonResult executeDataRule(Collateral collateral) {
			try {
				return exists(MapperS.of(collateral).<IndependentAmount>map("getIndependentAmount", _collateral -> _collateral.getIndependentAmount())).or(exists(MapperS.of(collateral).<ReferenceWithMetaCollateralPortfolio>mapC("getCollateralPortfolio", _collateral -> _collateral.getCollateralPortfolio()).<CollateralPortfolio>map("getValue", _f->_f.getValue())));
			}
			catch (Exception ex) {
