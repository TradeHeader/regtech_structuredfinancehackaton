...
public class ResourceLengthValidator implements Validator<ResourceLength> {
	...
	public ValidationResult<ResourceLength> validate(RosettaPath path, ResourceLength o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ResourceLength", ValidationType.CARDINALITY, "ResourceLength", path, "", error);
		}
		return success("ResourceLength", ValidationType.CARDINALITY, "ResourceLength", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ResourceLength", ValidationType.CARDINALITY, "ResourceLength", path, "", res.getError());
				}
				return success("ResourceLength", ValidationType.CARDINALITY, "ResourceLength", path, "");
			})
			.collect(toList());
