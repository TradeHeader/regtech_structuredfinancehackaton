...
public class AgreementNameTypeFormatValidator implements Validator<AgreementName> {
	...
	public ValidationResult<AgreementName> validate(RosettaPath path, AgreementName o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("AgreementName", ValidationType.TYPE_FORMAT, "AgreementName", path, "", error);
		}
		return success("AgreementName", ValidationType.TYPE_FORMAT, "AgreementName", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("AgreementName", ValidationType.TYPE_FORMAT, "AgreementName", path, "", res.getError());
				}
				return success("AgreementName", ValidationType.TYPE_FORMAT, "AgreementName", path, "");
			})
			.collect(toList());
