...
public class UmbrellaAgreementEntityTypeFormatValidator implements Validator<UmbrellaAgreementEntity> {
	...
	public ValidationResult<UmbrellaAgreementEntity> validate(RosettaPath path, UmbrellaAgreementEntity o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("UmbrellaAgreementEntity", ValidationType.TYPE_FORMAT, "UmbrellaAgreementEntity", path, "", error);
		}
		return success("UmbrellaAgreementEntity", ValidationType.TYPE_FORMAT, "UmbrellaAgreementEntity", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("UmbrellaAgreementEntity", ValidationType.TYPE_FORMAT, "UmbrellaAgreementEntity", path, "", res.getError());
				}
				return success("UmbrellaAgreementEntity", ValidationType.TYPE_FORMAT, "UmbrellaAgreementEntity", path, "");
			})
			.collect(toList());
