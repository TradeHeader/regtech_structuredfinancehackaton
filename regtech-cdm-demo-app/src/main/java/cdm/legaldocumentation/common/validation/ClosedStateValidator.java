...
public class ClosedStateValidator implements Validator<ClosedState> {
	...
	public ValidationResult<ClosedState> validate(RosettaPath path, ClosedState o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ClosedState", ValidationType.CARDINALITY, "ClosedState", path, "", error);
		}
		return success("ClosedState", ValidationType.CARDINALITY, "ClosedState", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ClosedState", ValidationType.CARDINALITY, "ClosedState", path, "", res.getError());
				}
				return success("ClosedState", ValidationType.CARDINALITY, "ClosedState", path, "");
			})
			.collect(toList());
