...
public class RepresentationsTypeFormatValidator implements Validator<Representations> {
	...
	public ValidationResult<Representations> validate(RosettaPath path, Representations o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Representations", ValidationType.TYPE_FORMAT, "Representations", path, "", error);
		}
		return success("Representations", ValidationType.TYPE_FORMAT, "Representations", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Representations", ValidationType.TYPE_FORMAT, "Representations", path, "", res.getError());
				}
				return success("Representations", ValidationType.TYPE_FORMAT, "Representations", path, "");
			})
			.collect(toList());
