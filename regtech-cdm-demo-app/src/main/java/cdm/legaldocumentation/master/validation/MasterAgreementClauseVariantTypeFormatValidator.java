...
public class MasterAgreementClauseVariantTypeFormatValidator implements Validator<MasterAgreementClauseVariant> {
	...
	public ValidationResult<MasterAgreementClauseVariant> validate(RosettaPath path, MasterAgreementClauseVariant o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MasterAgreementClauseVariant", ValidationType.TYPE_FORMAT, "MasterAgreementClauseVariant", path, "", error);
		}
		return success("MasterAgreementClauseVariant", ValidationType.TYPE_FORMAT, "MasterAgreementClauseVariant", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MasterAgreementClauseVariant", ValidationType.TYPE_FORMAT, "MasterAgreementClauseVariant", path, "", res.getError());
				}
				return success("MasterAgreementClauseVariant", ValidationType.TYPE_FORMAT, "MasterAgreementClauseVariant", path, "");
			})
			.collect(toList());
