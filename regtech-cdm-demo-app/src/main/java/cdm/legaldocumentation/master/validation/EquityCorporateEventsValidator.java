...
public class EquityCorporateEventsValidator implements Validator<EquityCorporateEvents> {
	...
	public ValidationResult<EquityCorporateEvents> validate(RosettaPath path, EquityCorporateEvents o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EquityCorporateEvents", ValidationType.CARDINALITY, "EquityCorporateEvents", path, "", error);
		}
		return success("EquityCorporateEvents", ValidationType.CARDINALITY, "EquityCorporateEvents", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EquityCorporateEvents", ValidationType.CARDINALITY, "EquityCorporateEvents", path, "", res.getError());
				}
				return success("EquityCorporateEvents", ValidationType.CARDINALITY, "EquityCorporateEvents", path, "");
			})
			.collect(toList());
