...
public class IndexAdjustmentEventsValidator implements Validator<IndexAdjustmentEvents> {
	...
	public ValidationResult<IndexAdjustmentEvents> validate(RosettaPath path, IndexAdjustmentEvents o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("IndexAdjustmentEvents", ValidationType.CARDINALITY, "IndexAdjustmentEvents", path, "", error);
		}
		return success("IndexAdjustmentEvents", ValidationType.CARDINALITY, "IndexAdjustmentEvents", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("IndexAdjustmentEvents", ValidationType.CARDINALITY, "IndexAdjustmentEvents", path, "", res.getError());
				}
				return success("IndexAdjustmentEvents", ValidationType.CARDINALITY, "IndexAdjustmentEvents", path, "");
			})
			.collect(toList());
