...
public class CollateralTransferAgreementElectionsValidator implements Validator<CollateralTransferAgreementElections> {
	...
	public ValidationResult<CollateralTransferAgreementElections> validate(RosettaPath path, CollateralTransferAgreementElections o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CollateralTransferAgreementElections", ValidationType.CARDINALITY, "CollateralTransferAgreementElections", path, "", error);
		}
		return success("CollateralTransferAgreementElections", ValidationType.CARDINALITY, "CollateralTransferAgreementElections", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CollateralTransferAgreementElections", ValidationType.CARDINALITY, "CollateralTransferAgreementElections", path, "", res.getError());
				}
				return success("CollateralTransferAgreementElections", ValidationType.CARDINALITY, "CollateralTransferAgreementElections", path, "");
			})
			.collect(toList());
