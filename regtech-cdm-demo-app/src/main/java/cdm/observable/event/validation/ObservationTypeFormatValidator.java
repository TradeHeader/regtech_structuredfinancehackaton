...
public class ObservationTypeFormatValidator implements Validator<Observation> {
	...
	public ValidationResult<Observation> validate(RosettaPath path, Observation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Observation", ValidationType.TYPE_FORMAT, "Observation", path, "", error);
		}
		return success("Observation", ValidationType.TYPE_FORMAT, "Observation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Observation", ValidationType.TYPE_FORMAT, "Observation", path, "", res.getError());
				}
				return success("Observation", ValidationType.TYPE_FORMAT, "Observation", path, "");
			})
			.collect(toList());
