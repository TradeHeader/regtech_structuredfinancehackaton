...
public class PriceScheduleValidator implements Validator<PriceSchedule> {
	...
	public ValidationResult<PriceSchedule> validate(RosettaPath path, PriceSchedule o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PriceSchedule", ValidationType.CARDINALITY, "PriceSchedule", path, "", error);
		}
		return success("PriceSchedule", ValidationType.CARDINALITY, "PriceSchedule", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceSchedule", ValidationType.CARDINALITY, "PriceSchedule", path, "", res.getError());
				}
				return success("PriceSchedule", ValidationType.CARDINALITY, "PriceSchedule", path, "");
			})
			.collect(toList());
