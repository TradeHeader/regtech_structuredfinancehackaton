...
public class ExchangeRateValidator implements Validator<ExchangeRate> {
	...
	public ValidationResult<ExchangeRate> validate(RosettaPath path, ExchangeRate o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ExchangeRate", ValidationType.CARDINALITY, "ExchangeRate", path, "", error);
		}
		return success("ExchangeRate", ValidationType.CARDINALITY, "ExchangeRate", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ExchangeRate", ValidationType.CARDINALITY, "ExchangeRate", path, "", res.getError());
				}
				return success("ExchangeRate", ValidationType.CARDINALITY, "ExchangeRate", path, "");
			})
			.collect(toList());
