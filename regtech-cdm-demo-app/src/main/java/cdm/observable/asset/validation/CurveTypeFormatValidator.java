...
public class CurveTypeFormatValidator implements Validator<Curve> {
	...
	public ValidationResult<Curve> validate(RosettaPath path, Curve o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Curve", ValidationType.TYPE_FORMAT, "Curve", path, "", error);
		}
		return success("Curve", ValidationType.TYPE_FORMAT, "Curve", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Curve", ValidationType.TYPE_FORMAT, "Curve", path, "", res.getError());
				}
				return success("Curve", ValidationType.TYPE_FORMAT, "Curve", path, "");
			})
			.collect(toList());
