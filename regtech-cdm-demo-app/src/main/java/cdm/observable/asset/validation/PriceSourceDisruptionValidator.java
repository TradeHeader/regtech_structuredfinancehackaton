...
public class PriceSourceDisruptionValidator implements Validator<PriceSourceDisruption> {
	...
	public ValidationResult<PriceSourceDisruption> validate(RosettaPath path, PriceSourceDisruption o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("PriceSourceDisruption", ValidationType.CARDINALITY, "PriceSourceDisruption", path, "", error);
		}
		return success("PriceSourceDisruption", ValidationType.CARDINALITY, "PriceSourceDisruption", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("PriceSourceDisruption", ValidationType.CARDINALITY, "PriceSourceDisruption", path, "", res.getError());
				}
				return success("PriceSourceDisruption", ValidationType.CARDINALITY, "PriceSourceDisruption", path, "");
			})
			.collect(toList());
