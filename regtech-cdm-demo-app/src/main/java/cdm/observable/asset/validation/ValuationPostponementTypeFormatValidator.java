...
public class ValuationPostponementTypeFormatValidator implements Validator<ValuationPostponement> {
	...
	public ValidationResult<ValuationPostponement> validate(RosettaPath path, ValuationPostponement o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ValuationPostponement", ValidationType.TYPE_FORMAT, "ValuationPostponement", path, "", error);
		}
		return success("ValuationPostponement", ValidationType.TYPE_FORMAT, "ValuationPostponement", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ValuationPostponement", ValidationType.TYPE_FORMAT, "ValuationPostponement", path, "", res.getError());
				}
				return success("ValuationPostponement", ValidationType.TYPE_FORMAT, "ValuationPostponement", path, "");
			})
			.collect(toList());
