...
public class CalculatedRateObservationDatesAndWeightsValidator implements Validator<CalculatedRateObservationDatesAndWeights> {
	...
	public ValidationResult<CalculatedRateObservationDatesAndWeights> validate(RosettaPath path, CalculatedRateObservationDatesAndWeights o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CalculatedRateObservationDatesAndWeights", ValidationType.CARDINALITY, "CalculatedRateObservationDatesAndWeights", path, "", error);
		}
		return success("CalculatedRateObservationDatesAndWeights", ValidationType.CARDINALITY, "CalculatedRateObservationDatesAndWeights", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculatedRateObservationDatesAndWeights", ValidationType.CARDINALITY, "CalculatedRateObservationDatesAndWeights", path, "", res.getError());
				}
				return success("CalculatedRateObservationDatesAndWeights", ValidationType.CARDINALITY, "CalculatedRateObservationDatesAndWeights", path, "");
			})
			.collect(toList());
