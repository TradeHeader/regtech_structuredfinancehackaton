...
public class CalculatedRateObservationsValidator implements Validator<CalculatedRateObservations> {
	...
	public ValidationResult<CalculatedRateObservations> validate(RosettaPath path, CalculatedRateObservations o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("CalculatedRateObservations", ValidationType.CARDINALITY, "CalculatedRateObservations", path, "", error);
		}
		return success("CalculatedRateObservations", ValidationType.CARDINALITY, "CalculatedRateObservations", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("CalculatedRateObservations", ValidationType.CARDINALITY, "CalculatedRateObservations", path, "", res.getError());
				}
				return success("CalculatedRateObservations", ValidationType.CARDINALITY, "CalculatedRateObservations", path, "");
			})
			.collect(toList());
