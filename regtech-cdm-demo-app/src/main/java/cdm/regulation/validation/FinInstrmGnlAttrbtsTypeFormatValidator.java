...
public class FinInstrmGnlAttrbtsTypeFormatValidator implements Validator<FinInstrmGnlAttrbts> {
	...
	public ValidationResult<FinInstrmGnlAttrbts> validate(RosettaPath path, FinInstrmGnlAttrbts o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FinInstrmGnlAttrbts", ValidationType.TYPE_FORMAT, "FinInstrmGnlAttrbts", path, "", error);
		}
		return success("FinInstrmGnlAttrbts", ValidationType.TYPE_FORMAT, "FinInstrmGnlAttrbts", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FinInstrmGnlAttrbts", ValidationType.TYPE_FORMAT, "FinInstrmGnlAttrbts", path, "", res.getError());
				}
				return success("FinInstrmGnlAttrbts", ValidationType.TYPE_FORMAT, "FinInstrmGnlAttrbts", path, "");
			})
			.collect(toList());
