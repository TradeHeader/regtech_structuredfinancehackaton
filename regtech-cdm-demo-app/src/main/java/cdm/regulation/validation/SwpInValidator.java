...
public class SwpInValidator implements Validator<SwpIn> {
	...
	public ValidationResult<SwpIn> validate(RosettaPath path, SwpIn o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SwpIn", ValidationType.CARDINALITY, "SwpIn", path, "", error);
		}
		return success("SwpIn", ValidationType.CARDINALITY, "SwpIn", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SwpIn", ValidationType.CARDINALITY, "SwpIn", path, "", res.getError());
				}
				return success("SwpIn", ValidationType.CARDINALITY, "SwpIn", path, "");
			})
			.collect(toList());
