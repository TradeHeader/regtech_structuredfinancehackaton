...
public class BuyrTypeFormatValidator implements Validator<Buyr> {
	...
	public ValidationResult<Buyr> validate(RosettaPath path, Buyr o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Buyr", ValidationType.TYPE_FORMAT, "Buyr", path, "", error);
		}
		return success("Buyr", ValidationType.TYPE_FORMAT, "Buyr", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Buyr", ValidationType.TYPE_FORMAT, "Buyr", path, "", res.getError());
				}
				return success("Buyr", ValidationType.TYPE_FORMAT, "Buyr", path, "");
			})
			.collect(toList());
