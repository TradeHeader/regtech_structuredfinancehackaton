...
public class FinInstrmValidator implements Validator<FinInstrm> {
	...
	public ValidationResult<FinInstrm> validate(RosettaPath path, FinInstrm o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("FinInstrm", ValidationType.CARDINALITY, "FinInstrm", path, "", error);
		}
		return success("FinInstrm", ValidationType.CARDINALITY, "FinInstrm", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("FinInstrm", ValidationType.CARDINALITY, "FinInstrm", path, "", res.getError());
				}
				return success("FinInstrm", ValidationType.CARDINALITY, "FinInstrm", path, "");
			})
			.collect(toList());
