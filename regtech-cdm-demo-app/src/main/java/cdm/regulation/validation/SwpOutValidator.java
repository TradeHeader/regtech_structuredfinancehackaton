...
public class SwpOutValidator implements Validator<SwpOut> {
	...
	public ValidationResult<SwpOut> validate(RosettaPath path, SwpOut o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SwpOut", ValidationType.CARDINALITY, "SwpOut", path, "", error);
		}
		return success("SwpOut", ValidationType.CARDINALITY, "SwpOut", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SwpOut", ValidationType.CARDINALITY, "SwpOut", path, "", res.getError());
				}
				return success("SwpOut", ValidationType.CARDINALITY, "SwpOut", path, "");
			})
			.collect(toList());
