...
public class InvstmtDcsnPrsnValidator implements Validator<InvstmtDcsnPrsn> {
	...
	public ValidationResult<InvstmtDcsnPrsn> validate(RosettaPath path, InvstmtDcsnPrsn o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("InvstmtDcsnPrsn", ValidationType.CARDINALITY, "InvstmtDcsnPrsn", path, "", error);
		}
		return success("InvstmtDcsnPrsn", ValidationType.CARDINALITY, "InvstmtDcsnPrsn", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("InvstmtDcsnPrsn", ValidationType.CARDINALITY, "InvstmtDcsnPrsn", path, "", res.getError());
				}
				return success("InvstmtDcsnPrsn", ValidationType.CARDINALITY, "InvstmtDcsnPrsn", path, "");
			})
			.collect(toList());
