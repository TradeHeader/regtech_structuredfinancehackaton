...
public class QtyTypeFormatValidator implements Validator<Qty> {
	...
	public ValidationResult<Qty> validate(RosettaPath path, Qty o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Qty", ValidationType.TYPE_FORMAT, "Qty", path, "", error);
		}
		return success("Qty", ValidationType.TYPE_FORMAT, "Qty", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Qty", ValidationType.TYPE_FORMAT, "Qty", path, "", res.getError());
				}
				return success("Qty", ValidationType.TYPE_FORMAT, "Qty", path, "");
			})
			.collect(toList());
