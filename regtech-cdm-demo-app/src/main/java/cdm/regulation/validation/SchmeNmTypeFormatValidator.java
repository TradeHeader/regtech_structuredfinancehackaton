...
public class SchmeNmTypeFormatValidator implements Validator<SchmeNm> {
	...
	public ValidationResult<SchmeNm> validate(RosettaPath path, SchmeNm o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("SchmeNm", ValidationType.TYPE_FORMAT, "SchmeNm", path, "", error);
		}
		return success("SchmeNm", ValidationType.TYPE_FORMAT, "SchmeNm", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("SchmeNm", ValidationType.TYPE_FORMAT, "SchmeNm", path, "", res.getError());
				}
				return success("SchmeNm", ValidationType.TYPE_FORMAT, "SchmeNm", path, "");
			})
			.collect(toList());
