...
public class OrdrTrnsmssnTypeFormatValidator implements Validator<OrdrTrnsmssn> {
	...
	public ValidationResult<OrdrTrnsmssn> validate(RosettaPath path, OrdrTrnsmssn o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("OrdrTrnsmssn", ValidationType.TYPE_FORMAT, "OrdrTrnsmssn", path, "", error);
		}
		return success("OrdrTrnsmssn", ValidationType.TYPE_FORMAT, "OrdrTrnsmssn", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("OrdrTrnsmssn", ValidationType.TYPE_FORMAT, "OrdrTrnsmssn", path, "", res.getError());
				}
				return success("OrdrTrnsmssn", ValidationType.TYPE_FORMAT, "OrdrTrnsmssn", path, "");
			})
			.collect(toList());
