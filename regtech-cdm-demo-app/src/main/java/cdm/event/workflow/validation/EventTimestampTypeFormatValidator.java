...
public class EventTimestampTypeFormatValidator implements Validator<EventTimestamp> {
	...
	public ValidationResult<EventTimestamp> validate(RosettaPath path, EventTimestamp o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("EventTimestamp", ValidationType.TYPE_FORMAT, "EventTimestamp", path, "", error);
		}
		return success("EventTimestamp", ValidationType.TYPE_FORMAT, "EventTimestamp", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("EventTimestamp", ValidationType.TYPE_FORMAT, "EventTimestamp", path, "", res.getError());
				}
				return success("EventTimestamp", ValidationType.TYPE_FORMAT, "EventTimestamp", path, "");
			})
			.collect(toList());
