...
public class TradeStateTypeFormatValidator implements Validator<TradeState> {
	...
	public ValidationResult<TradeState> validate(RosettaPath path, TradeState o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("TradeState", ValidationType.TYPE_FORMAT, "TradeState", path, "", error);
		}
		return success("TradeState", ValidationType.TYPE_FORMAT, "TradeState", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("TradeState", ValidationType.TYPE_FORMAT, "TradeState", path, "", res.getError());
				}
				return success("TradeState", ValidationType.TYPE_FORMAT, "TradeState", path, "");
			})
			.collect(toList());
