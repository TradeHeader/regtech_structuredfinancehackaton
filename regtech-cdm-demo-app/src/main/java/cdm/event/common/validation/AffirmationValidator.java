...
public class AffirmationValidator implements Validator<Affirmation> {
	...
	private List<ComparisonResult> getComparisonResults(Affirmation o) {
		return Lists.<ComparisonResult>newArrayList(
				...
				checkCardinality("status", (AffirmationStatusEnum) o.getStatus() != null ? 1 : 0, 1, 1)
			);
	}
	...
	public ValidationResult<Affirmation> validate(RosettaPath path, Affirmation o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Affirmation", ValidationType.CARDINALITY, "Affirmation", path, "", error);
		}
		return success("Affirmation", ValidationType.CARDINALITY, "Affirmation", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Affirmation", ValidationType.CARDINALITY, "Affirmation", path, "", res.getError());
				}
				return success("Affirmation", ValidationType.CARDINALITY, "Affirmation", path, "");
			})
			.collect(toList());
