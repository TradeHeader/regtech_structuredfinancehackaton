...
public class ContractFormationInstructionValidator implements Validator<ContractFormationInstruction> {
	...
	public ValidationResult<ContractFormationInstruction> validate(RosettaPath path, ContractFormationInstruction o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("ContractFormationInstruction", ValidationType.CARDINALITY, "ContractFormationInstruction", path, "", error);
		}
		return success("ContractFormationInstruction", ValidationType.CARDINALITY, "ContractFormationInstruction", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("ContractFormationInstruction", ValidationType.CARDINALITY, "ContractFormationInstruction", path, "", res.getError());
				}
				return success("ContractFormationInstruction", ValidationType.CARDINALITY, "ContractFormationInstruction", path, "");
			})
			.collect(toList());
