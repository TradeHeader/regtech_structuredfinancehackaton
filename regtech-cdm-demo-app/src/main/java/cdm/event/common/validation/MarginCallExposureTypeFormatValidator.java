...
public class MarginCallExposureTypeFormatValidator implements Validator<MarginCallExposure> {
	...
	public ValidationResult<MarginCallExposure> validate(RosettaPath path, MarginCallExposure o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("MarginCallExposure", ValidationType.TYPE_FORMAT, "MarginCallExposure", path, "", error);
		}
		return success("MarginCallExposure", ValidationType.TYPE_FORMAT, "MarginCallExposure", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("MarginCallExposure", ValidationType.TYPE_FORMAT, "MarginCallExposure", path, "", res.getError());
				}
				return success("MarginCallExposure", ValidationType.TYPE_FORMAT, "MarginCallExposure", path, "");
			})
			.collect(toList());
