...
public class InstructionTypeFormatValidator implements Validator<Instruction> {
	...
	public ValidationResult<Instruction> validate(RosettaPath path, Instruction o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Instruction", ValidationType.TYPE_FORMAT, "Instruction", path, "", error);
		}
		return success("Instruction", ValidationType.TYPE_FORMAT, "Instruction", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Instruction", ValidationType.TYPE_FORMAT, "Instruction", path, "", res.getError());
				}
				return success("Instruction", ValidationType.TYPE_FORMAT, "Instruction", path, "");
			})
			.collect(toList());
