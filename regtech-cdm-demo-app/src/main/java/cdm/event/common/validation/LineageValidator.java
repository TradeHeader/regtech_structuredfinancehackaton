...
public class LineageValidator implements Validator<Lineage> {
	...
	public ValidationResult<Lineage> validate(RosettaPath path, Lineage o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Lineage", ValidationType.CARDINALITY, "Lineage", path, "", error);
		}
		return success("Lineage", ValidationType.CARDINALITY, "Lineage", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Lineage", ValidationType.CARDINALITY, "Lineage", path, "", res.getError());
				}
				return success("Lineage", ValidationType.CARDINALITY, "Lineage", path, "");
			})
			.collect(toList());
