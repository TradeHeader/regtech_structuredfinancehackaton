	...
	public static class QuantityIncreasedDefault extends QuantityIncreased {
		...
		protected Boolean assignOutput(Boolean result, TradeState before, List<? extends TradeState> after) {
			result = areEqual(MapperC.<TradeState>of(after)
				.mapItem(item -> areEqual(MapperS.of(compareTradeLot.evaluate(item.<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<TradableProduct>map("getTradableProduct", trade -> trade.getTradableProduct()).<TradeLot>mapC("getTradeLot", tradableProduct -> tradableProduct.getTradeLot()).get(), CompareOp.GREATER_THAN, MapperS.of(before).<Trade>map("getTrade", tradeState -> tradeState.getTrade()).<TradableProduct>map("getTradableProduct", trade -> trade.getTradableProduct()).<TradeLot>mapC("getTradeLot", tradableProduct -> tradableProduct.getTradeLot()).get())), MapperS.of(true), CardinalityOperator.All).asMapper()), MapperS.of(true), CardinalityOperator.All).get();
