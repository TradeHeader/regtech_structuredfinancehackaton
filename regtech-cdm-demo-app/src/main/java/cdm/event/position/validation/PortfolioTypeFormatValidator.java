...
public class PortfolioTypeFormatValidator implements Validator<Portfolio> {
	...
	public ValidationResult<Portfolio> validate(RosettaPath path, Portfolio o) {
		...
		if (!isNullOrEmpty(error)) {
			return failure("Portfolio", ValidationType.TYPE_FORMAT, "Portfolio", path, "", error);
		}
		return success("Portfolio", ValidationType.TYPE_FORMAT, "Portfolio", path, "");
	}
		...
		return getComparisonResults(o)
			...
			.map(res -> {
				if (!isNullOrEmpty(res.getError())) {
					return failure("Portfolio", ValidationType.TYPE_FORMAT, "Portfolio", path, "", res.getError());
				}
				return success("Portfolio", ValidationType.TYPE_FORMAT, "Portfolio", path, "");
			})
			.collect(toList());
