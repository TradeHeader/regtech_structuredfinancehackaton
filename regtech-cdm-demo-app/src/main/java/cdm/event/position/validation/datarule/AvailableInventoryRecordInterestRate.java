		...
		private ComparisonResult executeDataRule(AvailableInventoryRecord availableInventoryRecord) {
			try {
				if (exists(MapperS.of(availableInventoryRecord).<Price>map("getInterestRate", _availableInventoryRecord -> _availableInventoryRecord.getInterestRate())).getOrDefault(false)) {
					return areEqual(MapperS.of(availableInventoryRecord).<Price>map("getInterestRate", _availableInventoryRecord -> _availableInventoryRecord.getInterestRate()).<PriceTypeEnum>map("getPriceType", priceSchedule -> priceSchedule.getPriceType()), MapperS.of(PriceTypeEnum.INTEREST_RATE), CardinalityOperator.All);
				}
